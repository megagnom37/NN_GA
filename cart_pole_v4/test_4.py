...
NUM_GENERATION = 1000
NUM_PARENT_NETWORKS = 6
CHILDREN_PER_PARENT = 4
NUM_MUTATION_WEIGHTS = 6000
MUTATION_FACTOR = np.float32(0.2)
MAX_REWARD = 500
...
def current_window_img(offset):
    ...
    # window_img = window_img.resize((int(window_img.size[0] / 2), 
    #                                 int(window_img.size[1] / 2)), 
    #                                 Image.BICUBIC)              
    return window_img
...
def generate_model(input_size):
    ...
    model.add(layers.Conv2D(4, (5, 5), activation='relu', input_shape=input_size))#, strides=(4, 4), padding='valid'))
